...
class Command(BaseCommand):
    help = 'Setup Google Drive integration'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Google Drive setup completed successfully!')
        )
        ...
        try:
            ...
            for field in required_fields:
                if field not in credentials_data:
                    raise CommandError(f"Missing required field in credentials: {field}")
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Credentials file copied to: {target_path}')
            )
            
        except json.JSONDecodeError:
            raise CommandError("Invalid JSON format in credentials file")
    ...
    def test_upload(self):
        ...
        try:
            ...
            with open(test_file_path, 'w') as f:
                f.write("This is a test file for Google Drive integration.")
            ...
            if result:
                ...
                gdrive_service.delete_file(result["file_id"])
                self.stdout.write("Test file cleaned up from Google Drive.")
            else:
